...
exports.modelChatRoom = (idRoom) => {
  return new Promise((resolve, reject) => {
    connection.query(`SELECT message_room.*, rooms.name as nameRoom, users.name as nameMember FROM message_room INNER JOIN rooms ON message_room.idRoom = rooms.id INNER JOIN users ON message_room.idUser = users.id WHERE rooms.id = '${idRoom}'`, (error, result) => {
      if (!error) {
        resolve(result)
      } else {
        reject(error)
